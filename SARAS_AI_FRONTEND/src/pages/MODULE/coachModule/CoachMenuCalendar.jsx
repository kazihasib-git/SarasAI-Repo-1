...
const CoachMenuCalendar = () => {
    ...
    useEffect(() => {
        if (timezoneDetails) {
            coachSessions?.length > 0 ? convertEvents() : setEventsList([]);
        }
    }, [timezoneDetails, coachSessions]);

    useEffect(() => {
        if (timezoneDetails) {
            coachSlots?.length > 0 ? convertSlots() : setSlotViewData([]);
        }
    }, [timezoneDetails, coachSlots]);
